...
[일정 생성 규칙]
1. **반드시 ${city} 행정구역 내 장소만 추천**. 다른 도시/지역 장소 절대 금지.
   - 예: 목적지가 "도쿄"면 오사카·교토·요코하마 장소 절대 금지.
   - 목적지가 "서울"이면 인천·수원 등 절대 금지.
   - 정확한 위치를 모르거나 ${city}인지 확신 없으면 그 장소는 반드시 제외할 것.
2. 유명하고 실제로 잘 알려진 장소만 포함 (불확실하면 제외, 가짜 장소 절대 만들지 말 것)
3. 하루 정확히 ${perDay}곳 (식당 포함)
4. **지역 클러스터링 (매우 중요)**:
   - 같은 날의 ${perDay}곳은 **1~2개의 인접한 동네/구 안에서만** 선택할 것.
   - 예시(도쿄): Day1=아사쿠사·우에노 권역, Day2=시부야·하라주쿠 권역, Day3=긴자·신주쿠 권역.
   - 하루 안에서 도시 반대편으로 점프하지 말 것 (대중교통 30분 이내 거리 유지).
5. **일자 간 동선 연결**:
   - Day N의 마지막 장소와 Day N+1의 첫 장소는 같은 동네이거나 인접한 동네여야 함.
   - 어제 시부야에서 끝났다면 오늘은 시부야/하라주쿠/오모테산도 등 근처에서 시작.
6. 반드시 점심(${schedule?.lunch ?? 13}:00 ± 30분)과 저녁(${schedule?.dinner ?? 19}:00 ± 30분) 시간대에 식당을 각 1곳씩 배치할 것
7. 첫 장소의 start_time은 반드시 ${(schedule?.start ?? 9) + (hotel ? 1 : 0)}:00으로 설정할 것 (기상 후 이동 시간 포함)
